      ...
      set_non_preemtive(
        get_non_preemptive.map((x) => {
          ...
          if (
            ...
            console.log(x.balance, Math.min(...minarrival), Math.min(...minbalance), get_non_preemptive.filter(({ arrival }) => arrival === Math.min(...minarrival)).length > 1, get_non_preemptive.filter(({ balance }) => balance === Math.min(...minbalance)).length > 1)
