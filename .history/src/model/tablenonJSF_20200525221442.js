      ...
      set_non_preemtive(
        get_non_preemptive.map((x) => {
          ...
          if (
            ...
            if (get_non_preemptive.filter(({ arrival }) => arrival === x.arrival).length > 1) {
              // กรณี arrival time มีค่าเท่ากัน
              if (get_non_preemptive.filter(({ burst }) => burst === x.burst).length > 1) {
                //กรณีที่ bust time เท่ากัน มันจะไปหา arrival time ที่นอยที่สุด
                if (x.balance === Math.min(...minarrival) && x.balance === Math.min(...minbalance)) {
                  x.balance = x.balance - 1;
                } else {
                  x.balance = x.balance - 1;
                }
              } else {
                if (x.balance === Math.min(...minbalance) && x.balance === Math.min(...minarrival)) {
                  x.balance = x.balance - 1;
                } else {
                  x.balance = x.balance - 1;
                }
