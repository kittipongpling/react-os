...
export default function TablenonJSF() {
  ...
  const handdleDel = (psx) => {
    ...
    set_non_preemtive(get_non_preemptive.filter(({
      ps
    }) => ps !== psx)); // เราดึงค่า  ps ตามเงื่อนไขของเรา 
          ...
          if (get_non_preemptive.filter(({
            ...
            if (x.balance === Math.min(...minbalance)) {
              if (get_non_preemptive.length > 0) {
                let result = get_non_preemptive.map(({
                  burst
                }) => burst).reduce((burst, i) => burst + i);
                console.log(result)
              ...
              if (x.balance > 0) {
                ...
                console.log(get_non_preemptive.reduce(({
                  burst
                }, i) => burst + i));
            ...
            if (x.arrival === Math.min(...minarrival)) {
              ...
              if (get_non_preemptive.length > 0) { // Process ... กำลังทำงาน
                let result = get_non_preemptive.map(({
                  burst
                }) => burst).reduce((burst, i) => burst + i);
                console.log(result)
              ...
              if (x.balance > 0) {
                ...
                if (x.balance === 1) { // process ... ทำงานเสร็จแล้ว
                  x.checkcolor = true;
                  x.status = 'Wait'
                  x.checkzie = false;
                  x.ct = x.burst
  ...
  return ( <
    ...
    TableCell > Burst Time & nbsp;
    (ms) < /TableCell> <
    TableCell > Balance & nbsp; < /TableCell> <
    TableCell > CT & nbsp; < /TableCell> <
    TableCell > TAT(ct - at) & nbsp; < /TableCell> <
    TableCell > WT(tat - bt) & nbsp; < /TableCell>

    <
    TableCell > deleteProcess & nbsp; < /TableCell> <
    /TableRow> <
    /TableHead> <
    TableBody > {
    ...
    onChange = {
      (e) => set_arrival(parseFloat(e.target.value))
    }
    id = "outlined-basic"
    label = "Arrival"
    variant = "outlined" /
    ...
    onChange = {
      (e) => set_burst(parseFloat(e.target.value))
    }
    id = "outlined-basic"
    label = "Burst"
    variant = "outlined" /
    ...
    /TableContainer> {
      get_sort_ct.filter(({
        ct
      }) => ct > 0).sort((a, b) => a ? .ct - b ? .ct).filter(({
        color
      }) => Object.keys(color).length > 0).map((ele) => {
        return ( <
          div style = {
            ele.color
          } > < /div>
