      ...
      set_non_preemtive(
        get_non_preemptive.map((x) => {
          ...
          if (
            ...
            if( get_non_preemptive.map(({burst}) => burst === 8 )){ //กรณีที่ bust time เท่ากัน มันจะไปหา arrival time ที่นอยที่สุด
            ...
            if (x.arrival === Math.min(...minarrival)) {
              ...
              if( get_non_preemptive.map(({ arrival }) => arrival === 0)){ // กรณีที่ arrival time เท่ากัน มันจะไปหา bust time ที่น้อยที่สุด 
                if(x.balance === Math.min(...minbalance)){
                  x.balance = x.balance - 1;
                }
              }else{
